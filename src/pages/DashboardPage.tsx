...
function toBreakdownItems(categories: ExpenseByCategory[], total: number): CategoryBreakdownItem[] {
...
export default function DashboardPage() {
    ...
    const loadCategoryBreakdown = async (monthToUse: number, yearToUse: number) => {
        ...
        try {
            ...
            const top = data.categories.reduce((max, c) => (c.amount > max.amount ? c : max), data.categories[0]);
            ...
            <div className="flex flex-col h-full min-h-0">
                ...
                <div className="flex flex-wrap items-end justify-between gap-3 mb-6 -mt-2">
                    <div className="flex flex-wrap items-end gap-3">
                        ...
                        <div className="space-y-1">
                            <label className="font-label-caps text-xs font-semibold text-on-surface block" htmlFor="dashboard-month">
                                Mês
                            </label>
                            <select
                                id="dashboard-month"
                        ...
                        {/* Year Select */}
                        <div className="space-y-1">
                            <label className="font-label-caps text-xs font-semibold text-on-surface block" htmlFor="dashboard-year">
                                Ano
                            </label>
                            <select
                                id="dashboard-year"
                ...
                <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-6 grid-rows-[auto_1fr]">
                    ...
                    <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6">
                        <StatCard label="Total de Despesas" sublabel="Este Mês" icon="account_balance" accentColor="bg-negative-rose">
                            <span className="font-display-lg  md:text-5xl text-4xl font-bold text-primary">
                                R${" "}
                        ...
                        <StatCard label="Despesas Fixas" sublabel="Valor pago no mês" icon="event_repeat">
                            <span className="font-title-md md:text-4xl text-3xl font-semibold text-primary ">
                                R${" "}
                                <span className="font-label-numeric">
                                    {fixedPaid.toLocaleString("pt-BR", {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2,
                                    })}
                                </span>
                        ...
                        <StatCard label="Despesas fixas em aberto" sublabel="Valor pendente no mês" icon="savings">
                            <span
                                className={`font-title-md md:text-4xl text-3xl  font-semibold  ${fixedPending > 0 ? "text-negative-rose" : "text-positive-emerald"}`}
                            >
                                R${" "}
                                <span className="font-label-numeric">
                                    {fixedPending.toLocaleString("pt-BR", {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2,
                                    })}
                                </span>
                    ...
                    <div className="lg:col-span-12 min-h-0 flex flex-col">
                        {loading ? (
                            <div className="bg-surface-container-lowest border border-border-subtle rounded-xl p-6 flex flex-col items-center justify-center flex-1 min-h-0">
                                ...
                                <p className="font-body-sm text-sm text-on-surface-variant mt-3">Carregando categorias...</p>
                            </div>
                        ) : error ? (
                            ...
                            <CategoryHorizontalBarChart items={breakdownItems} topCategoryLabel={topCategoryLabel} />
                        )}
                    </div>
                </div>
            </div>
